...
read_actions <- function() {
  ...
  usage_data <<- polloi::read_dataset("discovery/maps/actions_per_tool.tsv", col_types = "Dcci") %>%
...
# Read in user count
read_users <- function() {
  user_data <<- polloi::read_dataset("discovery/maps/users_per_feature.tsv", col_types = "Dci") %>%
...
read_tiles <- function() {
  new_tiles_automata <<- polloi::read_dataset("discovery/maps/tile_aggregates_with_automata.tsv", col_types = "Dcidcciidiii") %>%
    dplyr::filter(!is.na(total), !is.na(users)) %>%
    data.table::as.data.table()
  new_tiles_no_automata <<- polloi::read_dataset("discovery/maps/tile_aggregates_no_automata.tsv", col_types = "Dcidcciidiii") %>%
    dplyr::filter(!is.na(total), !is.na(users)) %>%
    data.table::as.data.table()
  return(invisible())
}
...
read_countries <- function(){
  country_data <<- polloi::read_dataset("discovery/maps/users_by_country.tsv", col_types = "Dcd") %>%
